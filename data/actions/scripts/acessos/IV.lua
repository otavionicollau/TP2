...
function onUse(player, item, fromPosition, itemEx, toPosition, isHotkey)
      ...
      player:registerEvent("modalTaskIV")

    if player:getStorageValue(15146) <= 0 then

         if player:getStorageValue(strong) >= 0 then

          local title = "ACESSO A AREA IV"
        ...
        local title = "ACESSO A AREA IV"
        local message = "PARABENS! \n\nVOCE TEM TOTAL ACESSO A AREA IV.\nAPROVEITE!\n\n\n                                   Att Baiak Server"
        local window = ModalWindow(1007, title, message)
        window:addButton(101, "Cancel")
        window:setDefaultEscapeButton(101)
        window:sendToPlayer(player)
